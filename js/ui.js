...
import { openLyricsPanel, clearFullscreenLyricsSync } from './lyrics.js';
import { mountSpicyFullscreen, updateSpicyFullscreen, isSpicyTakeoverOpen } from './spicy-mount.js';
import {
...
export class UIRenderer {
    ...
    async showFullscreenCover(track, nextTrack, lyricsManager, activeElement) {
        ...
        // The Spicy takeover owns the overlay in the lyrics-capable path: it
        // brings its own backdrop (DynamicBG) and transport (NowBar), so skip
        // Monochrome's video artwork + visualizer work entirely (they would
        // only render unseen behind it).
        const useSpicyTakeover = Boolean(
            lyricsManager && activeElement && lyricsPane && lyricsContent && track.type !== 'video'
        );
        const videoCoverRequest =
            !useSpicyTakeover && needsVideoCover
        ...
        const canRenderLyrics = useSpicyTakeover;
        if (canRenderLyrics) {
            if (lyricsToggleBtn) lyricsToggleBtn.style.removeProperty('display');
            overlay.classList.remove('lyrics-unavailable');
            if (isSpicyTakeoverOpen()) {
                // Track changed while the Spicy takeover is open: live-update
                // transport, backdrop and lyrics instead of tearing the page
                // down (teardown flashes and trips scroll measuring).
                await updateSpicyFullscreen(track, activeElement, lyricsManager, lyricsContent);
            } else {
                clearFullscreenLyricsSync(lyricsContent);
                await mountSpicyFullscreen(track, activeElement, lyricsManager, lyricsContent);
            }
        } else {

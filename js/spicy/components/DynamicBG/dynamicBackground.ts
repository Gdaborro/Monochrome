...
// MONO-PORT ADAPTATION: the original line below ran at module top level:
//     applyStaticBackgroundBlur($staticBackgroundBlur.get());
// This module and PageView import each other, so under native ESM that call
// can run before PageView's `let PageContainer` initializes (TDZ
// ReferenceError — even `?.` throws on an uninitialized binding). Deferred to
// a microtask; behavior is preserved, and every page open re-seeds via the
// "page:open" listener below. (Upstream bundles never hit this because their
// bundler orders modules differently.)

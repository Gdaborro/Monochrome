// Adapted from Spicy Lyrics src/utils/uiState.ts (see ../../ATTRIBUTION.md).
// Only change: Spicetify.LocalStorage -> window.localStorage. All atoms and
// the navigation observer identical.
...
function saveUiStateBlob(obj: Record<string, any>) {
    try {
        window.localStorage.setItem(UI_STATE_KEY, JSON.stringify(obj));
    } catch {
        /* storage may be unavailable; ui state simply won't persist */
    }
}
